...
class LTRTrainer(BaseTrainer):
    ...
    def _check_reins_gradients(self):
        """检查所有Reins参数的梯度状态"""
        has_grad = False
        print("\n===== Reins梯度验证 =====")
        for name, param in self.actor.net.named_parameters():
            if 'reins' in name.lower():
                if param.grad is None:
                    print(f"❌ 参数 {name} 无梯度")
                else:
                    has_grad = True
                    grad_data = param.grad.float() if param.grad.dtype == torch.float16 else param.grad
                    grad_mean = grad_data.mean().item()
                    print(f"✅ 参数 {name} 梯度存在，均值：{grad_mean:.6e}")

        if has_grad:
            print("===== Reins梯度验证通过 =====")
        else:
            print("⚠️警告：未检测到任何Reins参数的梯度！")
            import pdb;
            pdb.set_trace()
        print("============================\n")
