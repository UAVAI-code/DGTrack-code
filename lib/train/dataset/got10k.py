...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        构造函数，负责初始化类的实例
        args:
            root -GOT-10k 数据集的根目录路径。如果未提供，将使用默认路径
            image_loader (jpeg4py_loader) -  用于加载图像的函数，默认为 jpeg4py_loader
            split - 'train' or 'val'. 指定数据集的划分，可以是 'train'、'val' 或其他自定义分割
            seq_ids - 用于训练的序列 ID 列表
            data_fraction - 使用数据集的一部分的比例
        """
