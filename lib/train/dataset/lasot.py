...
class Lasot(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, vid_ids=None, split=None, data_fraction=None):
        ...
        args:
            ...
            root：LaSOT 数据集的根目录路径。如果未指定（None），则使用环境设置中的路径。
            image_loader：用于加载图像的函数，默认为 jpeg4py_loader。
            vid_ids：用于训练的视频ID列表。如果指定，将使用这些ID对应的视频进行训练。
            split：指定使用数据集的哪个分割（例如训练集或测试集）。只能与 vid_ids 之一一起使用。
            data_fraction：使用数据集的一部分，而不是整个数据集。
