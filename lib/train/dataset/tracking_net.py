...
def list_sequences(root, set_ids):
    ...
    列出给定 set_ids 中所有视频的路径，并返回一个包含这些视频信息的列表
    args:
        root: Root directory to TrackingNet;          TrackingNet 数据集的根目录路径
        set_ids: Sets (0-11) which are to be used;    要使用的 TrackingNet 数据集的集合 ID 列表，范围通常是 0 到 11
