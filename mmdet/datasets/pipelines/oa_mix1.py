...
from mmdet.datasets.pipelines.bbox_augmentation import (autocontrast, equalize, posterize, solarize, color, contrast, brightness, sharpness, invert,
...
class OAMix:
    ...
    def __call__(self, results, *args, **kwargs):
        ...
        for i in range(1, self.num_views + 1):
            if i == 1:
                ...
                results[f'oamix_boxes'] = np.stack(self._history["oa_random_box_list"], axis=0)
                results['custom_field'] += [f'img{i}', f'gt_bboxes{i}', f'oamix_boxes']
                results[f'multilevel_boxes'] = self._history["random_box_list"]
    ...
    def oamix(self, img, gt_bboxes):
        ...
        random_box_list, random_mask_list = self.get_random_regions(
            img,
            self.random_box_scale,
            self.random_box_ratio,
        ...
        img_oamix = self.object_aware_mixing(
            img,
            img_oamix,
            oa_target_mask_list,
            oa_target_score_list)
    ...
    def get_regions_for_object_aware_mixing(self, img, fg_box_list, fg_mask_list, fg_score_list):
        ...
        oa_random_box_list, oa_random_mask_list, oa_random_score_list = self.get_random_regions(
            img,
            self.oa_random_box_scale,
            self.oa_random_box_ratio,
            num_bboxes=min(max(len(oa_target_box_list), 1), 5),
            return_score=True,
            fg_box_list=fg_box_list,
            fg_score_list=fg_score_list
        )
    ...
    def aug(self, img, img_size, fg_box_list, fg_mask_list):
        ...
        if op in [autocontrast, equalize, posterize, solarize, color, contrast, brightness, sharpness]:
            ...
            pil_img = op(img,
                         level=self.severity,
                         img_size=img_size,
                         bboxes_xy=fg_box_list,
                         mask_bboxes=fg_mask_list,
                         fillmode='oa')
