...
class BaseDetector(BaseModule, metaclass=ABCMeta):
    ...
    def save_the_result_img(self, data):
        cls_scores_all = self.features['rpn_head.rpn_cls']
        ...
        labels_flatten, labels_flatten_weight = cls_reg_targets[0], cls_reg_targets[1]
        ...
        for i in range(5):
            ...
            label = label.reshape(num_type, featmap_sizes[i][0], featmap_sizes[i][1], -1).contiguous()
        ...
        for i in range(1, 5):
            ...
            labels_all[i] = F.interpolate(labels_all[i].type(torch.cuda.FloatTensor), size=(H, W), mode='nearest')
        cls_scores_all = torch.stack([cls_scores for cls_scores in cls_scores_all], dim=0)
        cls_scores_all = cls_scores_all.permute(2, 1, 0, 3, 4).contiguous()
        labels_all = torch.stack([labels for labels in labels_all], dim=0)
        labels_all = labels_all.permute(2, 1, 0, 3, 4).contiguous()

        num_priors = 3
        num_type = data['img'].size()[0]
        num_lev = 5
        plt.figure(figsize=(5 * (num_type + 1), 4 * num_priors))
        for p in range(num_priors):
            labels = labels_all[p]
            label = labels[0]
            label = label.sum(axis=0) / num_lev
            ...
            cls_scores = cls_scores_all[p]
            for t in range(num_type):
                cls_score = cls_scores[t]
                cls_score = cls_score.sum(axis=0) / num_lev
    ...
    def train_step(self, data, optimizer):
        ...
        data = integrate_data(data, self.train_cfg)
        ...
        self.update_wandb_features_log_vars(log_vars)
