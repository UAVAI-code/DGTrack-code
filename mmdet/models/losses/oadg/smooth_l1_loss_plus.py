...
def smooth_l1_loss(pred, target, num_views=3, beta=1.0, ):
    ...
    Args:
        pred (torch.Tensor): 模型预测值，形状为 [batch_size * num_views, ...].
        num_views :表示每个样本的多视图数量（如原始图像 + 增强图像）， 默认为3
        target (torch.Tensor): 目标值（Ground Truth），形状与 pred 一致.
        beta (float, optional): Smooth L1 Loss 的分段函数阈值，默认为 1.0，用于控制 L1 Loss 和 L2 Loss 的切换点。

    Returns:
        torch.Tensor: Calculated loss 返回的是计算的loss
    """
    ...
    loss_orig = torch.where(diff < beta, 0.5 * diff * diff / beta,diff - 0.5 * beta)
...
def jsdy(pred,
    ...
    if pred_orig.shape != label.shape:
        if pred_orig.shape[-1] == 1:
            ...
            label = F.one_hot(label, num_classes=pred_orig.shape[-1])
